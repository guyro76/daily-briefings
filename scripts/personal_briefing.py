"""
daily-personal-briefing - GitHub Actions version
Runs at 04:00 UTC (07:00 IL) and 15:00 UTC (18:00 IL)
Personal briefing: Israel news, tech/AI, photography
"""
...
NEWS_FEEDS = [
    ...
    "https://www.mako.co.il/rss/31750640610.xml",
]
TECH_AI_FEEDS = [
...
def main():
    ...
    news = get_news_items(NEWS_FEEDS, 3)
    tech = get_news_items(TECH_AI_FEEDS, 3, keywords=["AI","בינה","מלאכותי","GPT","טכנולוגיה","tech","אפליקציה"])
    photo = get_news_items(PHOTO_SOCIAL_FEEDS, 2, keywords=["photo","camera","צילום","Instagram","סושיאל","social"])

    # === CONTENT VALIDATION - abort if not enough real content ===
    is_valid, reason = validate_content(news, tech, photo)
    if not is_valid:
        print(f"SEND ABORTED - insufficient content: {reason}")
        print(f"News: {len(news)}, Tech: {len(tech)}, Photo: {len(photo)}")
        sys.exit(1)

    print(f"Content OK - sending ({len(news)+len(tech)+len(photo)} real items)")

    # Pad only after validation passes
    while len(news) < 3:
        news.append(("—", "אין עדכונים נוספים"))
    while len(tech) < 3:
        tech.append(("—", "אין עדכונים נוספים"))
    while len(photo) < 2:
        photo.append(("—", "אין עדכונים נוספים"))

    tips = [
        "קרא 10 דקות ביום — זה משנה לאורך זמן",
        "גבה את התמונות שלך לענן היום",
        "נסה prompt חדש ב-AI שלא ניסית קודם",
        "עדכן אפליקציה אחת שהזנחת",
        "צלם משהו שמסביבך — אפילו בסמארטפון",
        "שתף פוסט אחד שיכול לעזור לאחרים",
        "קח הפסקה מהמסך 10 דקות היום",
    ]
    ...
    plain = f"""{emoji} {greeting} - עדכון אישי {date_he}
...
body{{background:#f0f4ff;font-family:'Heebo',Arial,sans-serif;direction:rtl;text-align:right}}
.wrap{{padding:20px 12px}}
.card{{max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 8px 32px rgba(0,50,150,.12)}}
.hdr{{background:linear-gradient(135deg,#0d2b6e 0%,#1565c0 60%,#1e88e5 100%);padding:20px 24px 18px}}
.hdr-meta{{font-size:12px;font-weight:700;color:#bbdefb;letter-spacing:1px;margin-bottom:6px}}
.hdr-title{{font-size:24px;font-weight:900;color:#fff;margin-bottom:4px}}
.hdr-sub{{font-size:14px;font-weight:700;color:#e3f2fd}}
.stripe{{height:4px;background:linear-gradient(90deg,#0d2b6e,#1e88e5,#90caf9,#1e88e5,#0d2b6e)}}
.sec{{padding:18px 24px 14px;border-bottom:1px solid #e8f0fe}}
...
.sec-title{{font-size:17px;font-weight:900;color:#0d2b6e}}
.item{{margin-bottom:10px;background:#f0f4ff;border-radius:10px;border-right:4px solid #1565c0;padding:12px 14px}}
.item-label{{font-size:12px;font-weight:900;color:#1565c0;letter-spacing:1px;margin-bottom:4px}}
.item-text{{font-size:15px;color:#1a1a2e;line-height:1.65;font-weight:700}}
.tip{{margin:10px 24px 20px;background:#0d2b6e;border-radius:12px;padding:16px 18px}}
...
.ftr{{background:#f7f9ff;padding:14px 24px;text-align:center;font-size:12px;font-weight:700;color:#1565c0;border-top:2px solid #bbdefb}}
</style></head>
<body><div class="wrap"><div class="card">
<div class="hdr">
<div class="hdr-meta">&#x1F4C5; {date_he} | עדכון {session}</div>
<div class="hdr-title">{emoji} {greeting}</div>
<div class="hdr-sub">&#x1F1EE;&#x1F1F1; חדשות &#x2022; AI &#x2022; צילום</div>
</div>
<div class="stripe"></div>
<div class="sec">
<div class="sec-hdr"><span class="sec-icon">&#x1F4F0;</span><span class="sec-title">חדשות ישראל</span></div>
{html_items(news)}
</div>
<div class="sec">
<div class="sec-hdr"><span class="sec-icon">&#x1F916;</span><span class="sec-title">טכנולוגיה ו-AI</span></div>
{html_items(tech)}
</div>
<div class="sec">
<div class="sec-hdr"><span class="sec-icon">&#x1F4F8;</span><span class="sec-title">צילום וסושיאל</span></div>
{html_items(photo)}
</div>
<div class="tip">
...
<div class="ftr">גיא רוזנברג &#169;2026 | {footer}</div>
    ...
    subject = f"{emoji} עדכון אישי {session} | {date_dmy}"
    send_telegram(plain)
    try:
        send_whatsapp(plain)
    except Exception as e:
